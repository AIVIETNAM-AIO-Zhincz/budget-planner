# Standards for Tóm tắt tài chính tuần
...
> Áp dụng: `test_weekly.py` viết trước `weekly.py`; thường (net/Δ%/top/anomaly) + biên (1 tuần, rỗng,
> dưới floor, ổn định). Endpoint test happy + 401. Chatbot rule + mock. Template deterministic → assert
> chữ cố định an toàn.
...
> Áp dụng: `build_weekly_summary` thuần (nhận windows, không DB); không thêm dependency; ngưỡng là hằng.
...
> Áp dụng: `ANOMALY_FACTOR`/`MIN_FLOOR` (UPPER_SNAKE); `build_weekly_summary`/`weekly_windows`
> (snake_case); `WeeklySummary`/`WeeklyAnomaly` (PascalCase schema).
...
> Áp dụng: `GET /reports/weekly-summary` read-only, schema `WeeklySummary`, lọc `space_id` (qua
...
Slice không đổi schema — tính trên dữ liệu giao dịch có sẵn, **không** model/migration.
